    ...
    <template #actions>
      <div class="flex flex-wrap items-center justify-end gap-2">
        <details v-if="profiles.length" class="group relative">
          <summary class="app-button border border-green-300 bg-green-700 text-white text-[10px] font-bold uppercase tracking-wider transition-all active:scale-95 hover:bg-green-800 cursor-pointer px-3 py-1.5 list-none marker:hidden">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  <div class="min-w-0">
                    ...
                    <p class="mt-1 text-xs text-zinc-500">{{ formatProfileMeta(profile) }}</p>
                  </div>
                  <span class="shrink-0 text-xs font-medium text-zinc-500">{{ profile.items.length }} item</span>
                </div>
                <div v-if="profile.signature_image_path || profile.logo_image_path" class="mt-3 flex items-center gap-2">
                  <div v-if="profile.signature_image_path" class="flex h-12 w-20 items-center justify-center rounded-md border border-zinc-200 bg-white p-1">
                    <img :src="profile.signature_image_path" alt="Profile signature" class="max-h-10 max-w-full object-contain" />
                  </div>
                  <div v-if="profile.logo_image_path" class="flex h-12 w-12 items-center justify-center rounded-md border border-zinc-200 bg-white p-1">
                    <img :src="profile.logo_image_path" alt="Profile logo" class="max-h-10 max-w-full object-contain" />
                  </div>
                </div>
                <div class="mt-3 flex gap-2">
                  <button type="button" class="app-button app-button-primary !px-3 !py-1.5 text-xs" @click="useProfile(profile)">Use</button>
                  <button type="button" class="app-button app-button-danger !px-3 !py-1.5 text-xs" @click="emit('remove-profile', profile.id)">Remove</button>
                </div>
              </div>
            </div>
          </div>
...
import type { InvoiceFormState, ValidationErrors, ReusableProfileHistoryEntry } from '../types'
